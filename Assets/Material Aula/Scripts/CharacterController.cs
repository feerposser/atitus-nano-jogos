/*
 * Autor (git): @feerposser
 * Descrição: Player controller para side croller 2D feito para a disciplina de Tecnologias Emergentes
 * Ferramentas inclusas: move, dash, jump, wall slide, wall jump
 */
...
public class CharacterController : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        float ty = Mathf.Clamp(rb.linearVelocity.y, float.MinValue, 11);
        rb.linearVelocity = new Vector2(rb.linearVelocity.x, ty);
    ...
    private void ComputeShot()
    {
        ...
        {
            Debug.Log("Atirar");
            Shot shot = Instantiate(shotPrefab, shotPosition.position, Quaternion.identity).GetComponent<Shot>();
            shot.direction = new Vector2(move, 0);
        }
    }
    ...
    private void Jump(float verticalMultiplier = 0)
    {
        ...
        rb.AddForce(new Vector2(rb.linearVelocity.x, jumpForce * verticalMultiplier), ForceMode2D.Impulse);
    ...
    {
        rb.linearVelocity = new Vector2(rb.linearVelocity.x * horizontalMultiplier, rb.linearVelocity.y);
        if (rb.linearVelocity.y < 0) jumpState = JumpState.PREPARETOFALL;
    }
    ...
    private void JumpFalling()
    {
        ...
        if (fallingTime > fallingCounter && rb.linearVelocity.y > 0)
    ...
    private void Falling() =>
        rb.linearVelocity -= fallVectorGravity * fallMultiplier * Time.deltaTime;
    ...
    private void ExecuteJump()
    {
        ...
        if (jumpState.Equals(JumpState.DEFAULT) && rb.linearVelocity.y < 0)
    ...
    /* --- Start Move --- */
    private void Move() =>
        rb.linearVelocity = new Vector2(move * xSpeed, rb.linearVelocity.y);
